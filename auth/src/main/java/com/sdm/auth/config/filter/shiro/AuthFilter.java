package com.sdm.auth.config.filter.shiro;
...
import com.sdm.auth.common.utils.HttpContextUtil;
import com.sdm.auth.common.utils.TokenUtil;
import com.sdm.auth.model.shiro.AuthToken;
...
public class AuthFilter extends AuthenticatingFilter {
    ...
    protected boolean onLoginFailure(AuthenticationToken token, AuthenticationException e, ServletRequest request,
        ...
        httpResponse.setHeader("Access-Control-Allow-Origin", HttpContextUtil.getOrigin());
        httpResponse.setCharacterEncoding("UTF-8");
        httpResponse.setStatus(441);
        try {
            // 处理登录失败的异常
            Throwable throwable = e.getCause() == null ? e : e.getCause();
            Map<String, Object> result = new HashMap<>();
            result.put("status", 441);
